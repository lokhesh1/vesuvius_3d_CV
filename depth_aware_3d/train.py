...
from common.loss import compute_dice
from depth_aware_3d.config import CFG
from depth_aware_3d.model import create_model
from depth_aware_3d.loss import TopologyAwareLoss
...
class SurfaceDetectionModule(pl.LightningModule):
    def __init__(self):
        ...
        self.criterion = TopologyAwareLoss(dice_weight=0.4, topo_weight=CFG.topo_weight)
...
def main_train():
    ...
    checkpoint_cb = ModelCheckpoint(
        dirpath=CFG.output_dir, filename="proposed-{epoch}-{val_dice:.4f}",
        monitor="val_dice", mode="max", save_top_k=1,
    )
    early_stop_cb = EarlyStopping(monitor="val_dice", mode="max", patience=10)
