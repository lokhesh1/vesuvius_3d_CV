...
def predict_volume(model, vol_path, patch_size, stride, device):
    model.eval()
    vol = open_zarr_array(vol_path) if CFG.data_backend == "zarr" else open_volume_tif(vol_path)
    D, H, W = vol.shape[:3]
    ps = patch_size
...
def generate_submission(model):
    ...
    discover = discover_test_zarr if CFG.data_backend == "zarr" else discover_test_tif
    test_vols = discover(CFG.data_dir)
    if not test_vols:
